...
const reponseSchema = z.object({ envoiId: z.string().uuid() });

/**
 * Archive le document signé déposé par l'apprenant dans Google Drive et notifie le formateur.
 */
export const archiverReponseApprenant = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await supabaseAdmin.from("notifications").insert({
      ...
      message: `${apprenantLabel} a déposé « ${envoi.label} ». Le document est archivé dans Google Drive.`,
      lien: `/espace/dossiers/${envoi.dossier_id}`,
    });

    return { driveUrl: uploaded.webViewLink ?? folderUrl(targetFolderId), fileName };
  });
